...
def main(args):
    ...
    for ii in range(eval_num):
        ...
        print('save path {}'.format(save_video_path))

        ##### Save as frames
        # timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        # frame_save_path = os.path.join(save_video_path, 'sample' + '_' + args.ckpt.split('/')[-1][:-3] + '_' + timestamp + str(ii))
        ...
        # video_save_path = os.path.join(save_video_path, 'sample' + '_' + args.ckpt.split('/')[-1][:-3] + '_' + timestamp +'gt' + '.mp4')
        # print(video_save_path)
