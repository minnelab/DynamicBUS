...
def main(args):
    ...
    # Load and preprocess the condition image/video
    ...
    output_dir = args.save_video_path
    ii=0
    output_dir = args.save_video_path
    ii=0
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.lower().endswith('.png'):
                ...
                output_file = file.replace(".png",".mp4")
                # print(output_file)
                ...
                repeated_image_0 = image.repeat(1, args.num_frames, 1, 1, 1)
                if args.use_fp16:
                    repeated_image_0 = repeated_image_0.half().to(device)

                video_list = []
                # set_determinism(seed=1000) 
            
                # Perform VAE operation
                with torch.no_grad():
                    b, f, c, h, w = repeated_image_0.shape
                    repeated_image = rearrange(repeated_image_0, 'b f c h w -> (b f) c h w').contiguous()
                    latent = vae.encode(repeated_image).latent_dist.sample().mul_(0.18215)
                ...
                z[:,frame_num] = latent[:,frame_num] # TODO Ablation1 change condition position
                noise = z
                first_noise = z
                ...
                for i in range(Num):
                    ...
                    if using_cfg:
                        ...
                        y = torch.full((1,), 0, dtype=torch.long, device=device)
                        if 'malignant' in input_path:
                            y = torch.full((1,), 1, dtype=torch.long, device=device)
                        
                        y_null = torch.tensor([2] * 1, device=device)
                    ...
                    if args.sample_method == 'ddim':
                        ...
                        # print(frame_num)
                        if frame_num >= 0:
                            samples = diffusion.ddim_sample_loop_i2v(
                            sample_fn, frame_num, i, z.shape,samples_last_step, z, condition_frame_num, anchor_frame, clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=device
                            )
                        else:
                            samples = diffusion.ddim_sample_loop_video(
                        ...
                        samples = diffusion.p_sample_loop_video(
                            model.to(device), z.shape, z, clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=device
                        )
                        # z = noise
                        # z = noise
                ...
                # Save final video
                # save_video_path =  os.path.join(args.save_video_path,  str(y[0].item()))
                # if not os.path.exists(save_video_path):
                #     os.makedirs(save_video_path)
                
                # #### Save as video 
                # timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
                # video_save_path = os.path.join(save_video_path, 'sample' + '_' + args.ckpt.split('/')[-1][:-3] + '_' + timestamp + str(ii) +'.mp4')
                print(output_path)
                imageio.mimwrite(output_path, final_video, fps=8, quality=9)
                print('save path {}'.format(output_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--frame_num", type=int, default=None) # any frame condition on the $frame_num$th frame
    parser.add_argument("--input_dir", type=str, default="/home/extradisk/liuyaofang/datasets/BUSI_crop_resize'")
    args = parser.parse_args()
    omega_conf = OmegaConf.load(args.config)
    omega_conf.ckpt = args.ckpt
    omega_conf.save_video_path = args.save_video_path
    omega_conf.frame_num = args.frame_num
    omega_conf.input_dir = args.input_dir
    main(omega_conf)
